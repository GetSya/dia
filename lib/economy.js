...
const CONFIG = {
    ...
    SHOP_ITEMS: [
        // Items - Money
        { id: 'energy_refill', name: 'Energy Refill', price: 1000, currency: 'money', stock: -1, desc: 'Isi ulang energi bermain', category: 'Items' },
        { id: 'lucky_ticket', name: 'Lucky Ticket', price: 5000, currency: 'money', stock: -1, desc: 'Tingkatkan luck mancing 1 jam', category: 'Boost' },
        { id: 'xp_boost', name: 'XP Boost', price: 10000, currency: 'money', stock: -1, desc: 'Double XP 2 jam', category: 'Boost' },
        { id: 'bait_premium', name: 'Bait Premium Pack', price: 7500, currency: 'money', stock: -1, desc: '10x Umpan Premium', category: 'Game Upgrade' },
        { id: 'rod_repair_kit', name: 'Rod Repair Kit', price: 3000, currency: 'money', stock: -1, desc: 'Repair pancingan 50%', category: 'Game Upgrade' },
        // Point items
        { id: 'premium_ticket', name: 'Premium Ticket', price: 50, currency: 'point', stock: -1, desc: 'Akses fitur premium 3 hari', category: 'Premium Feature' },
        { id: 'special_item', name: 'Special Item', price: 100, currency: 'point', stock: 100, desc: 'Item langka koleksi', category: 'Special Items' },
        { id: 'cosmetic_legend', name: 'Cosmetic Legend', price: 200, currency: 'point', stock: 50, desc: 'Cosmetic eksklusif', category: 'Cosmetics' },
        { id: 'werewolf_rolepack', name: 'Werewolf Role Pack', price: 150, currency: 'point', stock: -1, desc: 'Unlock role pack werewolf', category: 'Game Upgrade' }
    ],
...
function _ensureEconomy() {
    ...
    if (!db.economy.shopStocks) db.economy.shopStocks = {}
...
function getShopItems() {
    const eco = _ensureEconomy()
    return CONFIG.SHOP_ITEMS.map(it => {
        const stock = it.stock === -1 ? -1 : (it.stock - (eco.shopStocks[it.id] || 0))
        return { ...it, remaining: it.stock === -1 ? '∞' : Math.max(0, stock) }
    })
}
function getShopItem(id) {
    return CONFIG.SHOP_ITEMS.find(i => i.id === id) || null
}
async function purchaseShop(jid, itemId, qty = 1) {
    qty = Math.floor(Number(qty) || 1)
    if (qty < 1) throw new Error('Qty minimal 1')
    if (qty > 100) throw new Error('Qty maksimal 100')
    ...
    return _withLock(jid, async () => {
        const eco = _ensureEconomy()
        const w = eco.wallets[jid] || _getWalletRaw(jid)
        // stock check
        if (item.stock !== -1) {
            const sold = eco.shopStocks[item.id] || 0
            if (sold + qty > item.stock) throw new Error(`Stok habis. Sisa ${item.stock - sold}`)
        }
        ...
        if (item.currency === 'money') {
            if ((w.money || 0) < total) {
                const err = new Error('INSUFFICIENT')
                err.code = 'INSUFFICIENT'
                err.need = total; err.have = w.money || 0
                throw err
            }
            const before = w.money
            w.money -= total
            w.total_spent = (w.total_spent || 0) + total
            w.updated_at = _isoNow()
            _syncToUser(jid)
            _recordTransaction({ user_id: jid, type: 'SHOP_PURCHASE', amount: -total, currency: 'money', description: `Shop ${item.name} x${qty}`, before, after: w.money })
        } else {
            if ((w.point || 0) < total) {
                const err = new Error('INSUFFICIENT_POINT')
                err.code = 'INSUFFICIENT_POINT'
                err.need = total; err.have = w.point || 0
                throw err
            }
            const before = w.point
            w.point -= total
            w.updated_at = _isoNow()
            _syncToUser(jid)
            _recordTransaction({ user_id: jid, type: 'SHOP_PURCHASE', amount: -total, currency: 'point', description: `Shop ${item.name} x${qty}`, before, after: w.point })
        }
        // give inventory
        if (!w.inventory) w.inventory = {}
        w.inventory[item.id] = (w.inventory[item.id] || 0) + qty
        if (item.stock !== -1) eco.shopStocks[item.id] = (eco.shopStocks[item.id] || 0) + qty
        joDatabase.saveDB(true)
        return { wallet: { ...w }, item, qty, total }
    })
}
...
module.exports = {
    ...
    getShopItems, getShopItem, purchaseShop,
