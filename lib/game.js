...
// Guard: satu array game hanya diawasi satu interval, berapa pun
// seringnya cekWaktuGame dipanggil (control.js memanggilnya tiap pesan).
const _watched = new WeakSet()
exports.cekWaktuGame = (conn, _dir) => {
    if (!_dir || _watched.has(_dir)) return
    _watched.add(_dir)
    setInterval(() => {
      let position = null
