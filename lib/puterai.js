...
async function chat(key, text, { who = '', model = MODEL } = {}) {
    ...
    // cegah impersonasi: pushname yang mengandung "jojo"/"bot" jangan dipakai mentah
    let safeWho = String(who || '').trim()
    if (/jojo|bot/i.test(safeWho)) safeWho = 'User'
    if (safeWho.length > 20) safeWho = safeWho.slice(0, 20)
    const puter = await getPuter()
    const userLine = safeWho ? `${safeWho}: ${text}` : String(text)
    ...
    reply = stripNamePrefix(reply, safeWho)
    // cegah balasan 1 huruf tidak bermakna (mis. "L") — paksa ulang dengan konteks
    if (reply.length === 1 && /^[a-z]$/i.test(reply)) {
        // jangan push history untuk balasan sampah, coba minta penjelasan
        reply = 'Maksudnya "L" yang mana nih? Kalau "L" slang bisa berarti "lah" atau "lose", tergantung konteks wkwk.'
    }
