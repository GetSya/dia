...
function sfilemobiSearch(query, page = 1) {
  return new Promise(async (resolve, reject) => {
    ...
    $("div > div > div > div.list").each((_, el) => {
      ...
      const type =
        (_a = /\/smallicon\/(.*?)\.svg/.exec(icon)) === null || _a === void 0
          ? void 0
          : _a[1];
      const filesizeH =
        (_b = /\((.*?)\)/.exec($el.text())) === null || _b === void 0
          ? void 0
          : _b[1];
...
function sfilemobi(url) {
  return new Promise(async (resolve, reject) => {
    ...
    if (!html) {
      ...
      const $ = cheerio.load(html.data);
      const $k =
        (_a = /var z = (.*?);/i.exec($.html())) === null || _a === void 0
          ? void 0
          : _a[1];
      const urlPage =
        (((_d =
          ((_b = /var db = "(.*?)"/i.exec($.html())) === null || _b === void 0
            ? void 0
            : _b[1]) ||
          ((_c = /var sf = "(.*?)"/i.exec($.html())) === null || _c === void 0
            ? void 0
            : _c[1])) === null || _d === void 0
          ? void 0
          : _d.replace(/\\(\\)?/gi, "")) || $("#download").attr("href")) +
        `&k=${$k}`;
      const filename =
        $("div.intro-container > img").attr("alt") ||
        $("div.intro-container > h1").text();
      const icon = $("div.intro-container > img").attr("src");
      const type =
        (_e = /\/smallicon\/(.*?)\.svg/.exec(icon)) === null || _e === void 0
          ? void 0
          : _e[1];
      const $list = $("div.list");
      const mimetype =
        (_f = $list.eq(0).text().split("-")[1]) === null || _f === void 0
          ? void 0
          : _f.trim();
      const aploud =
        (_g = $list.eq(2).text().split("Uploaded:")[1]) === null ||
        _g === void 0
          ? void 0
          : _g.trim();
      ...
      const decs = $(
        "body > div.w3-row-padding.w3-container.w3-white > div > div:nth-child(1) > div:nth-child(6) "
      ).text();
      const downloads = parseInt(
        (_h = $list.eq(3).text().split("Downloads:")[1]) === null ||
          _h === void 0
          ? void 0
          : _h.trim()
      );
      const filesizeH =
        (_j = /\((.*?)\)/i.exec($("#download").text())) === null ||
        _j === void 0
          ? void 0
          : _j[1];
      const filesize = filesizeH && (0, parseFileSize)(filesizeH);
...
function wallpaper(title, page = "1") {
  return new Promise((resolve, reject) => {
    ...
    axios
      .get(
        `https://www.besthdwallpaper.com/search?CurrentPage=${randomgambar}&q=${title}`
      )
      .then(({ data }) => {
...
function covid() {
  return new Promise(async (resolve, reject) => {
    axios
      .get("https://covid19.go.id/")
      .then(({ data }) => {
        const $ = cheerio.load(data);
        const hasil = [];
        $("#case > div > div > div > div > div:nth-child(2)").each(function (
          a,
          b
        ) {
          ...
          const upindo = $(b)
            .find("div.pt-4.text-color-black.text-1")
            .text()
            .trim();
          $("#case > div > div > div > div > div:nth-child(1)").each(function (
            c,
            d
          ) {
            ...
            const up = $(d)
              .find("div.pt-4.text-color-grey.text-1")
              .text()
              .trim();
            const result = {
              indo: {
...
function komiku(judul) {
  return new Promise(async (resolve, reject) => {
    axios
      .get(
        "https://data3.komiku.id/cari/?post_type=manga&s=" +
...
function surah(no) {
  return new Promise(async (resolve, reject) => {
    axios
      .get("https://kalam.sindonews.com/surah/" + no)
      .then(({ data }) => {
        const $ = cheerio.load(data);
        ...
        var ya = $(
          "body > main > div.container > div.content.clearfix > div.news.col-md-9 > section > div.list-content.clearfix > div:nth-child(1) > div.ayat-title > h1"
        ).text();
        if (!ya) {
          resolve();
        } else {
...
function sholat(NO) {
  return new Promise(async (resolve, reject) => {
    axios
      .get("https://kalam.sindonews.com/jadwalsholat/" + NO)
      .then(({ data }) => {
        const $ = cheerio.load(data);
        const result = {};
