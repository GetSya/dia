...
const economy = (()=>{ try{ return require('./economy')} catch{ return null } })()
...
async function cmdMancing(bob, m, ctx) {
  ...
  try {
    ...
    // sinkron wallet money legacy tetap lewat economy? user.money sudah di wallet
    saveFish()
    // Reward ekonomi: bonus Money berdasarkan rarity (sink seimbang)
    try {
      if (economy) {
        const bonus = economy.mancingRewardForFish(fish)
        economy.giveGameReward(m.sender, 'mancing', { amount: bonus, idempotency: `fish_${Date.now()}_${Math.floor(Math.random()*1000)}` }).catch(()=>{})
      }
    } catch {}
...
async function cmdBuyumpan(bob, m, ctx) {
  ...
  // pakai economy agar tercatat & atomic
  if (economy) {
    try { await economy.removeMoney(m.sender, totalCost, 'SHOP_PURCHASE', `Buy ${matchedBait.name} x${qty}`) } catch(e){
      if(e.code==='INSUFFICIENT') return reply(`*Uang tidak cukup!*\nHarga total: Rp ${formatMoney(totalCost)}\nSaldo: Rp ${formatMoney(e.have)}\nKurang: Rp ${formatMoney(e.need - e.have)}`)
      throw e
    }
    // refresh user ref after economy deduction
    const db2 = joDatabase.loadDB()
    const fresh = db2.users[m.sender]
    if (fresh) user.money = fresh.money
  } else {
...
async function cmdJualikan(bob, m, ctx) {
  ...
  if (isBotTarget) {
    ...
    if (parsed.mode === 'all') {
      ...
      // lewat economy agar tercatat
      if (economy) {
        try { await economy.addMoney(m.sender, totalEarned, 'GAME_REWARD', `Jual ${totalItems} ikan ke bot`) } catch {}
        const db3 = joDatabase.loadDB(); const u3 = db3.users[m.sender]; if(u3) user.money = u3.money
      } else {
    ...
    const net = gross - tax
    if (economy) {
      try { await economy.addMoney(m.sender, net, 'GAME_REWARD', `Jual ${item.name} x${qty} ke bot`) } catch {}
      const db4 = joDatabase.loadDB(); const u4 = db4.users[m.sender]; if(u4) user.money = u4.money
    } else {
...
async function cmdRepairing(bob, m, ctx) {
  ...
  if (economy) {
    try { await economy.removeMoney(m.sender, repairCost, 'SHOP_PURCHASE', `Repair rod ${oldRod}% -> 100%`) } catch(e){
      if(e.code==='INSUFFICIENT') return reply(`*Uang tidak cukup untuk repair!*\nBiaya: Rp ${formatMoney(repairCost)}\nSaldo: Rp ${formatMoney(e.have)}\nKurang: Rp ${formatMoney(e.need - e.have)}`)
      throw e
    }
    const db2 = joDatabase.loadDB(); const f2 = db2.users[m.sender]; if(f2) user.money = f2.money
  } else {
...
async function handleSellResponse(bob, m, rawText) {
  ...
  if (txt === 'accept' || txt === 'terima') {
    ...
    // gunakan economy jika ada agar tercatat, fallback ke direct
    let ecoSuccess = false
    if (economy) {
      try {
        // fee 0 untuk jual ikan peer (sink sudah via pajak bot)
        await economy.transferMoney(m.sender, session.sellerId, session.totalPrice).then(()=>{ ecoSuccess = true }).catch(()=>{})
        // economy.transferMoney sudah potong fee 2% -> kita butuh transfer tanpa fee, jadi jika success, refund fee
        if (ecoSuccess) {
          // kembalikan fee yang terpotong (karena ikan trade harus 0 fee)
          const feeRefund = Math.max(economy.CONFIG.TRANSFER_FEE_MIN, Math.floor(session.totalPrice * economy.CONFIG.TRANSFER_FEE_RATE))
          try { await economy.addMoney(m.sender, feeRefund, 'REFUND', 'Refund fee ikan peer') } catch {}
          // sesuaikan seller/buyer ref setelah economy
          const db2 = joDatabase.loadDB()
          seller.money = db2.users[session.sellerId]?.money || seller.money
          buyer.money = db2.users[m.sender]?.money || buyer.money
        }
      } catch {}
    }
    if (!ecoSuccess) {
    ...
    // sinkron economy wallet jika masih desync
    if (economy && !ecoSuccess) {
      try {
        const ecoDb = joDatabase.loadDB()
        if (ecoDb.economy && ecoDb.economy.wallets) {
          if (ecoDb.economy.wallets[m.sender]) ecoDb.economy.wallets[m.sender].money = buyer.money
          if (ecoDb.economy.wallets[session.sellerId]) ecoDb.economy.wallets[session.sellerId].money = seller.money
        }
      } catch {}
    }
