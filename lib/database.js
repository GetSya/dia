...
// ========== SETTING GRUP (welcome/left/antilink/mute/chatbot) ==========
function getGroup(jid) {
    const db = loadDB()
    if (!db.groups) db.groups = {}
    if (!jid) return {}
    if (!db.groups[jid]) {
        db.groups[jid] = { welcome: false, left: false, antilink: false, mute: false, chatbot: false, antidelete: false, welcomeText: '', leftText: '' }
    }
