...
// ── TTS: jawaban AI -> voice note (model nahida) ──
async function ttsNahidaUrl(text) {
    ...
    const nahida = res.data?.result?.find?.((r) => r?.model === 'nahida' && r?.url)
...
async function ttsVoiceNote(text) {
    ...
    try {
        ...
        await execFileAsync('ffmpeg', ['-y', '-loglevel', 'error', '-i', inWav, '-c:a', 'libopus', '-b:a', '48k', '-f', 'ogg', outOgg])
...
module.exports = { handleTagbot, isAiReply, rememberAiReply }
