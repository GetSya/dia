...
const addPremiumUser = (userId, expired, _dir) => {
    ...
    if (expired === 'PERMANENT') {
        ...
        expired_at = Date.now() + runtime(expired)
    }

    const obj = { id: userId, expired: expired_at }
    _dir.push(obj)
    fs.writeFileSync('./assets/db/premium.json', JSON.stringify(_dir, null, 2))
}
 ...
 * @param {Object} _dir 
 */
// Guard: interval cek expired hanya dibuat sekali, berapa pun
// seringnya expiredCheck dipanggil (control.js memanggilnya tiap pesan).
let _expiredWatching = false
const expiredCheck = (conn, _dir) => {
    if (_expiredWatching) return
    _expiredWatching = true
    setInterval(() => {
        let position = null
        Object.keys(_dir).forEach((i) => {
        ...
        if (position !== null) {
            ...
            let txt = `PREMIUM KAMU SUDAH HABIS TERIMA KASIH TELAH BERLANGGANAN DI JOJO-BOT`
