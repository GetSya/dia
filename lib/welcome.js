...
async function sendWelcomeGoodbye(bob, gid, action, participants, opts = {}) {
    const gset = joDatabase.getGroup(gid)
    ...
    for (const p of (participants || [])) {
        ...
        try {
            ...
            try {
                ...
                console.log('[welcome] gagal render card:', err?.message || err)
            }
            ...
            if (cardBuf) {
                await bob.sendMessage(gid, { image: cardBuf, caption, mentions: [r.jid] })
            } else {
                await bob.sendMessage(gid, { text: caption, mentions: [r.jid] })
            }
            sent++
